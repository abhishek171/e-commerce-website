        ...
        for(let j = 0;j<10;j++){
            ...
            for (let k in productData[i][j]) {
                if (k == "images") {
                    ...
                    div2Child1.innerHTML ="Electronics Items";
...
function addToWishtList(imgSrc,title,price,stock){
    ...
    localStorage.setItem("whishListValues",whishListArr);
...
function closeFilter(e){
    ...
    if(filterApplied.length > 0){
        if(filterName == "Smart Phones" || filterName == "Laptops"){
            if(filterName == "Smart Phones"){
                if(filterApplied.includes("Laptops")){
                    Array.from(allColms).filter((a)=>{
                        return a.className == "col smartphones";
                    }).map((b)=>{
                ...
                if(filterApplied.includes("Smart Phones")){
                    Array.from(allColms).filter((a)=>{
                        return a.className == "col laptops";
                    }).map((b)=>{
                        b.style.display = "none";
                    });
                    filterApplied.filter((a)=>{
                        return a == "Smart Phones";
                    }).map((b)=>{
                        filterApplied.filter((c)=>{
                            return c!=b;
                        }).map((d)=>{
        ...
        if(optionType == "sorting-option"){
            if(!document.getElementById("filter-list").hasChildNodes()){
                ...
                if(optionName == "price" && optionValue == "hightolow"){
                    ...
                    for(let i in productData){
                        arr = productData[i].slice(0,10).sort((a,b)=>{
            ...
            if(filterApplied[filterApplied.length-1] == "Smart Phones"){
                if(filterApplied.length == 1){
                    let allCols = document.querySelectorAll("#products .col");
                    let arr1 = Array.from(allCols).filter((a)=>{
                        return a.getAttribute("class") != "col smartphones";
                    }).map((b)=>{
                    ...
                    if(filterApplied.includes("Laptops")){
                        if(filterApplied.length == 2){
                            ...
                            let allCols = document.querySelectorAll("#products .col");
                            filterApplied.filter((a)=>{
                                return a != "Smart Phones" || a != "Laptops";
                            }).map((b)=>{
                        ...
                        filterApplied.filter((a)=>{
                            ...
                            Array.from(allCols).filter((b)=>{
                                return b.getAttribute("name") == a  && b.className == "col laptops";
                            }).map((c)=>{
                                console.log(c);
            ...
            }else if(filterApplied[filterApplied.length-1] == "Laptops"){
                if(filterApplied.length == 1){
                    let allCols = document.querySelectorAll("#products .col");
                    let arr1 = Array.from(allCols).filter((a)=>{
                        return a.getAttribute("class") != "col laptops";
                    }).map((b)=>{
                        return b.style.display = "none";
                    });
                    document.getElementById("result-count").innerHTML = "Showing  "+ (10-arr1.length) +" of 30 results";
                    ...
                    if(filterApplied.includes("Smart Phones")){
                        ...
                        filterApplied.filter((a)=>{
                            ...
                            Array.from(allCols).filter((a)=>{
                            return a.getAttribute("name") == b && a.className == "col smartphones";
                            }).map((b)=>{
                                return b.style.display = "none";
                            })
                        });
