...
if(user.style.background == "" && sessionStorage.getItem("name") ==""){
    user.style.background = `url(${sessionStorage.getItem("avatar")}`;
...
if (!products.hasChildNodes()) {
    for(let i in productData){
        for(let j = 20;j<30;j++){
            ...
            div2.setAttribute(
                "style",
                "position:absolute;top:70%;left:38%;background-color:white;color:black;font-weight:700;display:flex;flex-direction:column;justify-content:center;align-items:center;"
            );
...
function closeFilter(e){
    ...
    if(filterApplied.length > 0){
        if(filterName == "Home-Decoration" || filterName == "Groceries"){
            if(filterName == "Home-Decoration"){
                if(filterApplied.includes("Groceries")){
                    Array.from(allColms).filter((a)=>{
                        return a.className == "col home-decoration";
                    }).map((b)=>{
                ...
                if(filterApplied.includes("Home-Decoration")){
                    Array.from(allColms).filter((a)=>{
                        return a.className == "col groceries";
                    }).map((b)=>{
                        b.style.display = "none";
                    });
                    filterApplied.filter((a)=>{
                        return a == "Home-Decoration";
                    }).map((b)=>{
                        filterApplied.filter((c)=>{
                            return c!=b;
                        }).map((d)=>{
...
function closeAllFilter(e){
    ...
    products.innerHTML = "";
        ...
        document.getElementById("result-count").innerHTML = "Showing 21-30 of 30 results";
}
        ...
        if(optionType == "sorting-option"){
            if(!document.getElementById("filter-list").hasChildNodes()){
                ...
                if(optionName == "price" && optionValue == "hightolow"){
                    ...
                    for(let i in productData){
                        arr = productData[i].slice(20,30).sort((a,b)=>{
            ...
            if(filterApplied[filterApplied.length-1] == "Home-Decoration"){
                if(filterApplied.length == 1){
                    let allCols = document.querySelectorAll("#products .col");
                    let arr1 = Array.from(allCols).filter((a)=>{
                        return a.getAttribute("class") != "col home-decoration";
                    }).map((b)=>{
                    ...
                    if(filterApplied.includes("Groceries")){
                        if(filterApplied.length == 2){
                            ...
                            let allCols = document.querySelectorAll("#products .col");
                            filterApplied.filter((a)=>{
                                return a != "Home-Decoration" || a != "Groceries";
                            }).map((b)=>{
                        ...
                        filterApplied.filter((a)=>{
                            ...
                            Array.from(allCols).filter((b)=>{
                                return b.getAttribute("name") == a  && b.className == "col groceries";
                            }).map((c)=>{
            ...
            }else if(filterApplied[filterApplied.length-1] == "Groceries"){
                if(filterApplied.length == 1){
                    let allCols = document.querySelectorAll("#products .col");
                    let arr1 = Array.from(allCols).filter((a)=>{
                        return a.getAttribute("class") != "col groceries";
                    }).map((b)=>{
                        return b.style.display = "none";
                    });
                    document.getElementById("result-count").innerHTML = "Showing  "+ (10-arr1.length) +" of 30 results";
                    ...
                    if(filterApplied.includes("Home-Decoration")){
                        ...
                        filterApplied.filter((a)=>{
                            ...
                            Array.from(allCols).filter((a)=>{
                            return a.getAttribute("name") == b && a.className == "col home-decoration";
                            }).map((b)=>{
                                return b.style.display = "none";
                            })
                        });
                ...
                document.getElementById("result-count").innerHTML = "Showing  "+ finalCount.length +" of 30 results";
                }else if(filterApplied.includes("Home-Decoration") || filterApplied.includes("Groceries")){
                    if(filterApplied.includes("Home-Decoration") && filterApplied.includes("Groceries")){
                        let allCols = document.querySelectorAll("#products .col");
                        filterApplied.filter((a)=>{
                            return a == "Home-Decoration";
                        }).map((b)=>{
